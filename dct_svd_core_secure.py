...
def embed(cover_path: str,
    ...
    ok = cv2.imwrite(out_path, stego, [cv2.IMWRITE_PNG_COMPRESSION, 0])
    if not ok: raise IOError("Ghi stego thất bại.")
    # Trong embed, pipeline GRAY (dùng cho text/json/ảnh xám)
    np.savez_compressed(meta_path, mode='gray', payload_type=payload_type,
                        Sc=Sc, Uw=Uw, Vwt=Vwt, Sw=Sw,  # <-- THÊM Sw VÀO ĐÂY
                        shape=(H, W), alpha=alpha)

    # Trong embed, pipeline IMAGE GRAY
    np.savez_compressed(meta_path, mode='gray', payload_type='image',
                        Sc=Sc, Uw=Uw, Vwt=Vwt, Sw=Sw,  # <-- THÊM Sw VÀO ĐÂY
                        shape=(H, W), alpha=alpha)

    # Trong embed, pipeline COLOR
    np.savez_compressed(meta_path, mode='color', payload_type='image',
                        Sb=Sb, Sg=Sg, Sr=Sr,
                        UWb=UWb, VWbt=VWbt, SWb=SWb,  # <-- THÊM SWb
                        UWg=UWg, VWgt=VWgt, SWg=SWg,  # <-- THÊM SWg
                        UWr=UWr, VWrt=VWrt, SWr=SWr,  # <-- THÊM SWr
                        shape=(H, W), alpha=alpha)
...
def extract(stego_path: str, meta_path: str, out_path: str, normalize: bool=True) -> str:
    ...
    if payload_type in ('text','json'):
        ...
        Sw_hat[K:] = 0  # suppress high-index noise
...
def detect(stego_path: str, meta_path: str, thresh: float = 0.6) -> Tuple[bool, float]:
    ...
    def _nc(v1, v2):
        ...
        v1 = v1.flatten().astype(np.float32)
        v2 = v2.flatten().astype(np.float32)
        if v1.size != v2.size or v1.size == 0:
            return 0.0
        # Normalize vectors to have zero mean and unit variance
        v1_norm = v1 - np.mean(v1)
        v2_norm = v2 - np.mean(v2)
        denom1 = np.sqrt(np.sum(v1_norm ** 2))
        denom2 = np.sqrt(np.sum(v2_norm ** 2))
        if denom1 < 1e-9 or denom2 < 1e-9:
            return 0.0
        return np.dot(v1_norm, v2_norm) / (denom1 * denom2)

    if mode == 'gray':
        Y, _ = _to_Y(st)
        Cw = dct2(Y)
        _, S_cw, _ = np.linalg.svd(Cw, full_matrices=False)
        Sc = data['Sc']
        Sw_orig = data['Sw']  # Lấy Sw gốc
        L = min(len(Sc), len(S_cw), len(Sw_orig))

        # Trích xuất Sw_hat
        Sw_hat = (S_cw[:L] - Sc[:L]) / max(alpha, 1e-8)

        # Tính NC
        score = _nc(Sw_orig[:L], Sw_hat)
    ...
    Sb, Sg, Sr = data['Sb'], data['Sg'], data['Sr']
    SWb_orig, SWg_orig, SWr_orig = data['SWb'], data['SWg'], data['SWr']

    Lb = min(len(Sb), len(S_cwb), len(SWb_orig))
    Lg = min(len(Sg), len(S_cwg), len(SWg_orig))
    Lr = min(len(Sr), len(S_cwr), len(SWr_orig))

    SWb_hat = (S_cwb[:Lb] - Sb[:Lb]) / max(alpha, 1e-8)
    SWg_hat = (S_cwg[:Lg] - Sg[:Lg]) / max(alpha, 1e-8)
    SWr_hat = (S_cwr[:Lr] - Sr[:Lr]) / max(alpha, 1e-8)

    nc_b = _nc(SWb_orig[:Lb], SWb_hat)
    nc_g = _nc(SWg_orig[:Lg], SWg_hat)
    nc_r = _nc(SWr_orig[:Lr], SWr_hat)
